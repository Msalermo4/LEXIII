...
export async function GET(req: NextRequest) {
  ...
  for (let y = start; y <= end; y++) {
    ...
    for (const m of anchors) {
      ...
      const fechaTxt = (block.match(/Fecha\s+([0-9]{1,2}\s+de\s+\w+\s+de\s+20\d{2})/i) || [])[1] || null;
      const materia = (block.match(/Materia\s+(.+)$/i) || [])[1] || '';

      // remove leading "In re:"
      const name = partes.replace(/^in\s*re\s*:?\s*/i, '').trim();

      // naive date parse fallback (Spanish months won’t parse reliably; store null if invalid)
