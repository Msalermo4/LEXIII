...
export default async function Page({ params }:{ params:{ id:string } }){
  ...
  const atty = await prisma.attorney.findUnique({ where:{ id }, include:{ actions:{ orderBy:{ decisionDate:'desc' } } } })
  ...
  return (<div>
    ...
    <div style={{display:'grid', gridTemplateColumns:'1fr 2fr', gap:12, marginTop:8}}>
      <div style={{padding:16, border:'1px solid #e5e7eb', borderRadius:8, background:'#fff'}}>
        ...
        {atty.statusEffectiveDate ? <div><b>Vigente desde:</b> {new Date(atty.statusEffectiveDate).toLocaleDateString('es-PR')}</div> : null}
      </div>
      <div style={{padding:16, border:'1px solid #e5e7eb', borderRadius:8, background:'#fff'}}>
        <h2 style={{fontSize:16, fontWeight:700, marginBottom:8}}>Línea de tiempo disciplinaria</h2>
        {atty.actions.length===0 ? <div style={{fontSize:14, color:'#6b7280'}}>Sin acciones registradas.</div> :
        <ul style={{display:'grid', gap:8, listStyle:'none', padding:0}}>
          {atty.actions.map(ac=> (
            <li key={ac.id} style={{border:'1px solid #e5e7eb', borderRadius:8, padding:12}}>
              <div style={{fontSize:12, color:'#6b7280'}}>{ac.decisionDate ? new Date(ac.decisionDate).toLocaleDateString('es-PR') : '—'} • <b>{ac.actionType}</b></div>
